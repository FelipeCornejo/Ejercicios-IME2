...
#Como el experimento emplea dos muestras a ser comparadas y además solicitan verificar 
#que algoritmo genético es más eficiente, se considera adecuada la implementación de 
#la prueba de rangos Wilcoxon-Mann-Whitney. A continuación se comprueban las condiciones para
#esta prueba:
#1. Dado que pertenece a un estudio realizado para un trabajo de título, se asume
#que las observaciones de las muestras son independientes.
#2. La escala de medición de las observaciones es continua.
#3. En vista de que nos piden determinar si un algoritmo más eficiente que otro, la
...
#HA: El algoritmo A es más eficiente que el algoritmo B.
#El gráfico demuestra que ambas muestras no poseen una distribución normal, pero dada
...
#puede rechazar la hipótesis nula en favor de la hipótesis alternativa, por lo que
#no hay diferencias en los tiempos de ejecución de cada algoritmo y ambos poseen la misma
#velocidad.
...
# Como valor p es 0.01228 mayor que nuestro nivel de significación 0.05, se rechaza la hipotesis nula a favor de la alternativa, es decir que con un 95%
# confianza, existe almenos una muestra la cual su media es distinta de las demás.

# Para ello se realizará un procedimiento post - hoc el cual nos indicarán las diferencias significativas.
...
# tasa de interés, se quiere saber visualizar el efecto que genera, mediante los precios
# de productos, para verificar si hay un aumento equitativo en los valores en porcentajes
#de cada banco según el servicio que disponen a sus clientes.
#H0: todos los servicios aumentaron su precio en cantidades similares.
#HA: Al menos un banco incrementó el precio en su servicios de manera exorbitante.
